...
export class AS2Parser {
  ...
  private static transformNodeLike (nodeLike: any, rootNode?: AS2MimeNode, parentNode?: AS2MimeNode): AS2MimeNode {
    const currentNode = new AS2MimeNode({
      ...
      content: nodeLike.content === undefined || this.isStream(nodeLike.content) ? nodeLike.content : Buffer.from(nodeLike.content),
  ...
  static async parse (content: Buffer | Stream | string): Promise<AS2MimeNode> {
    if (this.isStream(content)) content = await this.streamToBuffer(content as Stream)
